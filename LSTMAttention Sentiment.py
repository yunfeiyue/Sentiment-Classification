...
class LSTMAttention(nn.Module):
    ...
    def forward(self, text):
        ...
        output, (hidden, cell) = self.lstm(embedded)  # output: [batch size, sent len, hid dim]

        # Attention mechanism
        attention_weights = torch.softmax(self.attention(output).squeeze(2), dim=1)  # [batch size, sent len]
        context = torch.bmm(attention_weights.unsqueeze(1), output).squeeze(1)  # [batch size, hid dim]
...
model = LSTMAttention(INPUT_DIM, EMBEDDING_DIM, HIDDEN_DIM, OUTPUT_DIM, N_LAYERS, DROPOUT, PAD_IDX)
